...
# Secrets and dependency scan (diff-only)
...
mkdir -p "$OUTPUT_DIR"
JSON_OUT="$OUTPUT_DIR/secrets-scan.json"
COMMENT_OUT="$OUTPUT_DIR/secrets-scan-comment.md"
...
DIFF_RAW=$(git -C "$WORKDIR" diff --no-color "${BASE_REF}"...HEAD --unified=3 || true)
...
payload=$(jq -n \
  ...
  '{model:$model,temperature:0,max_tokens:1200,
    messages:[
      ...
      {role:"user",content:("Scan the git diff for hardcoded secrets, tokens, credentials, and risky dependency changes. Respond ONLY in required JSON.\n```\n"+$diff+"\n```")}
    ]}')
    ...
    local)
      if [[ "${LLM_ENDPOINT:-http://localhost:11434/api/generate}" == *"/generate"* ]]; then
        gen_payload=$(jq -n \
          ...
          --arg prompt "$(cat "$PROMPT_FILE")" \
          --arg diff "$TRIMMED_DIFF" \
          '{model:$model,prompt:($prompt+"\n\nScan this git diff for secrets and supply-chain risks. Respond in JSON only:\n```\n"+$diff+"\n```"),stream:false}')
...
echo "Secrets scan complete (max severity: $MAX_SEV)"
